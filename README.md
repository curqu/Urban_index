...
# Instructions for Use
  ...
  ### Input Layers
  Input layers from the datasets listed in the dependencies section include a population layer and a "built up" layer from the GHSL dataset; and an accessibility layer from MAP. The user should ensure that they data they select is at a 250m resolution, and from the correct year. All three inout layers should be clipped to the study area and reprojected as appropriate before running the script. The user should change the following layer names (in quotations) to match their clipped inputs:
